...
- Run `amen_rt.exe test_native/test.wav`; `z`/`x` decrease/increase speed by 5%, `c` resets speed to 100%, Space retriggers, `b` opens the browser, `j`/`k` navigate, Enter loads a WAV, Backspace moves to the parent folder, and `q` exits. The existing mode/effect/BPM controls remain available outside the browser. Adding strict warnings to this target currently emits warnings from vendored `miniaudio.h`, unlike the engine-only tests.
