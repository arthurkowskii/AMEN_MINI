...
- Run `amen_rt.exe test_native/test.wav`. The keyboard simulates the front panel: numpad 1-6 = voice pads (press = trigger the break, hold = SD browser opens), numpad 7-9 = FX pads (hold = FX screen, BLANK by default), F1-F7 select the active encoder, arrows turn the selected encoder, Enter clicks it, Space retriggers the last played pad, Backspace moves to the parent folder, and `q` exits. E1 is the fixed navigation encoder: on a held voice pad it browses the SD card (Enter enters a folder or loads a WAV), on a held FX pad it browses the FX list (Enter assigns the FX to the pad, navigating to BLANK unassigns). E2 = effect amount, E3 = effect cycle, E4 = speed ±5 % (click = 100 %), E5 = mode, E6 = reserved for J10, E7 = BPM. The encoder simulation is Windows-only; the POSIX path keeps the legacy keys. Adding strict warnings to this target currently emits warnings from vendored `miniaudio.h`, unlike the engine-only tests.
