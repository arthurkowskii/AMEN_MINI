...
void testPatterns() {
...

    {
        ...
        SimpleMidiController controller;
        assert(controller.page() == amen::PerformancePage::None && controller.tempo() == 120);
...

    {
        ...
        assertEvents(press(withSlot, 13), {on(73)});
        assertEvents(release(withSlot, 0), {off(60)});
        assertEvents(release(withSlot, 13), {off(73)});
...

    {
        ...
        assertEvents(press(controller, 0, 0), {on(60)});
        press(controller, 12, 0);
        assertEvents(tick(controller, 125), {off(60), on(61)});
        assertEvents(togglePage(controller), {off(61), on(60)});
        assert(controller.page() == amen::PerformancePage::None && !controller.runActive());
        assertEvents(release(controller, 12), {});
        release(controller, 0);
    }

    {
        ...
        expected.drawText(0, 11, "CHROMATIC", 2);
        expected.drawText(9, 22, "(12 SEMITONES)", 2);
...
int main() {
...

    {
        SimpleMidiController controller;
        g_block = "basics";
        constexpr std::array<uint8_t, 12> cChromatic{{60, 61, 62, 63, 64, 65, 66, 67, 68, 69, 70, 71}};
        ...
        assert(controller.highestNote() == 71);
        for (uint8_t key = 0; key < controller.kKeyCount; ++key) {
            assertNoteOns(press(controller, key), {cChromatic[key]});
            assertEvents(release(controller, key), {off(cChromatic[key])});
        }
...

    {
        ...
        assertEvents(press(controller, 2), {on(62)});
        assert(std::string(controller.currentNoteName()) == "D");
        assertEvents(release(controller, 2), {off(62)});
        assert(std::string(controller.currentNoteName()) == "C");
        assertEvents(release(controller, 0), {off(60)});
        assert(controller.currentNoteName()[0] == '\0');
    }
