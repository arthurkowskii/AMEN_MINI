...
Le clic E3 alterne `HARMONY` / `PATTERN` / `NONE` : en `PATTERN`, les huit pads supérieurs portent huit patterns assignables — `RUN UP`, `RUN DOWN`, `UP DOWN`, `DOWN UP`, `THIRDS UP`, `THIRDS DN`, `ARP UP`, `ARP DOWN` — et la rotation E3 réassigne le slot du pad tenu. En `NONE`, les huit pads supérieurs jouent les huit degrés suivants de la gamme, note à note. Le déclenchement marche dans les deux ordres : note inférieure tenue puis pad supérieur, ou pad supérieur tenu puis note inférieure. Le pattern remplace temporairement la voix de la source et la restaure en fin de run si elle est encore tenue ; un seul run joue à la fois, avec transfert transactionnel. Les assignations vivent en RAM et reviennent aux valeurs par défaut au redémarrage. E4 règle le pas de 30 à 200 ms (80 ms initialement). Revenir en `HARMONY` ou `NONE` annule le run ; les rôles des pads tenus, l'ownership partagé et le contexte figé sont préservés. Voir `docs/CONTROLS.md` pour les règles complètes.
